...
export function pages() {
  return (['pt', 'en'] as const).flatMap(language => {
    const prefix = language === 'en' ? '/en' : '';
    const root = language === 'en' ? '/help' : '/ajuda';
    return [
...
export function render(page: Page) {
  ...
  const homeTitle = en ? 'Condominium management software in Portugal | Habitae' : 'Software de gestão de condomínios em Portugal | Habitae';
  const homeDescription = en ? 'Habitae brings finances, fees, documents, people and tasks together in one clear condominium management platform.' : 'O Habitae reúne finanças, quotas, documentos, pessoas e tarefas numa plataforma clara para a gestão de condomínios.';
  const appDescription = en ? 'Join the Habitae waitlist to hear when our condominium management software launches in Portugal. Sign up as an administrator or resident.' : 'Entre na lista de espera do Habitae para saber quando o software de gestão de condomínios fica disponível em Portugal. Para administradores e moradores.';
  ...
  const title = kind === 'blog' ? `${post?.title ?? blogIndex[language].title} | Habitae` : trust ? `${trust.title} | Habitae` : legal ? `${legal.title} | Habitae` : article ? `${article.title} · ${en ? 'Habitae Help Centre' : 'Ajuda Habitae'}` : kind === 'help' ? (en ? 'Habitae Help Centre' : 'Ajuda Habitae') : kind === 'app' ? (en ? 'Join the Habitae waitlist' : 'Lista de espera Habitae') : homeTitle;
  const description = post?.description ?? (kind === 'blog' ? blogIndex[language].description : undefined) ?? trust?.description ?? legal?.description ?? article?.excerpt ?? (kind === 'app' ? appDescription : kind === 'help' ? (en ? 'Step-by-step guides to get started and manage your condominium in Habitae.' : 'Guias passo a passo para começar e gerir o seu condomínio no Habitae.') : homeDescription);
  const pathname = page.path.replace(/^\/en\//, '/');
  ...
  const alternate = (lang: Language) => {
    ...
    if (kind === 'help') {
      ...
      return `/${lang === 'en' ? 'help' : 'ajuda'}/${translated ? `${translated.slug}/` : ''}`;
    }
    return `${lang === 'en' ? '/en' : ''}${pathname}`;
  };
  const url = `https://habitae.pt${page.path}`;
  const breadcrumb = { '@type': 'BreadcrumbList', itemListElement: [
    { '@type': 'ListItem', position: 1, name: 'Habitae', item: `https://habitae.pt${en ? '/en/' : '/'}` },
    ...(kind === 'home' ? [] : [{ '@type': 'ListItem', position: 2, name: kind === 'blog' ? 'Blog' : kind === 'help' ? (en ? 'Help' : 'Ajuda') : title, item: kind === 'blog' ? `https://habitae.pt${blogPath(language)}` : kind === 'help' ? `https://habitae.pt/${en ? 'help' : 'ajuda'}/` : url }]),
    ...((post || article) ? [{ '@type': 'ListItem', position: 3, name: (post || article)!.title, item: url }] : []),
  ] };
  const structuredData = { '@context': 'https://schema.org', '@graph': [
    organizationSchema,
    { '@type': 'WebSite', '@id': 'https://habitae.pt/#website', name: 'Habitae', alternateName: 'habitae.pt', url: 'https://habitae.pt/', inLanguage: ['pt-PT', 'en'], publisher: { '@id': 'https://habitae.pt/#organization' } },
