...
export function signupEmailPayload(kind, signup, env) {
  ...
  const title = en ? 'You’re on the list.' : 'Está na lista.';
  const paragraphs = en ? [
  ...
  const action = { href: `https://habitae.pt/${en ? 'en/' : ''}blog/`, label: en ? 'Read the condominium blog' : 'Ler o blog do condomínio' };
  return {
    from: env.RESEND_FROM, to: [signup.email], reply_to: env.RESEND_REPLY_TO,
    subject: en ? 'You’re on the Habitae waitlist' : 'Está na lista de espera do Habitae',
    text: `Habitae\n\n${title}\n\n${paragraphs.join('\n\n')}\n\n${action.label}: ${action.href}\n\n${footer}\n\n${en ? 'Privacy' : 'Privacidade'}: https://habitae.pt/${en ? 'en/' : ''}privacy/#waitlist`,
